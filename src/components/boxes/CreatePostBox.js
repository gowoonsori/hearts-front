...
const CreatePostBox = () => {
  ...
  const createPost = useCallback(async()=> {
    ...
    setTags([]);
  },[axios,content,search,selectCategory,tags,postList,setPostList,setContent]);
    ...
    <Box sx={{ minWidth: '280px', display: 'block', backgroundColor: 'primary.main', m: 1, mb: 3, borderRadius: '10px', p: 2, width: '98.4%',pb:7 }}>
      <form sx={{ width: '100%' }}>
        <Box sx={{ display: 'inline-flex', width: '100%', mb: 2 }}>
          ...
          <TextField onChange={onChangeContent} value={content}required sx={{ ml: 1, width: '95%', backgroundColor: 'primary.second', borderRadius: '5px' }} placeholder="문구 작성" color="secondary" multiline />
        </Box>
        <Box>
          {selectCategory && (
        ...
        <Box sx={{ display: 'flex', mt: 5 }}>
          ...
          {search ?
            <Button onClick={onChangeSearchEvent} sx={{ mx:1, borderRadius: '20px',  border: '1px solid', borderColor: '#ff585b', backgroundColor: '#ff585b', color: 'primary.main' , fontWeight: 600, '&:hover':{borderColor: '#ff585b', backgroundColor: '#ff585b', color: 'primary.main'}}}>
              <CheckIcon/>
              검색 노출
            </Button>
            :
        ...
        <Button onClick={createPost} sx={{  float:'right', borderRadius: '20px', width: '300px', backgroundColor: 'primary.contrastText', color: 'secondary.main', fontWeight: 800, '&:hover':{backgroundColor:'primary.second'} }}>작성하기</Button>
      </form>
